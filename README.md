# Learning
[View this practice page](https://zzuiding.github.io)
## HTMLearning
...
在css应用规则到同一个元素时，会应用靠后的规则
同时会优先选择更为具体的元素

#### 继承

一些设置在父元素上的 CSS 属性是可以被子元素继承的，有些则不能。
只有部分属性能够被继承到其子类之中，需要注意其关系所在

#### 优先级选择

依次分为id，类，元素选择三个种类，其中id>类>元素，即但凡存在高一阶的选择器，则会无条件使用高一阶的选择器（即更为详细）
同时，若同类的选择器值（权重更高），也会优先使用该选择器（即选择器的界定分量更多，更为详细的确定是哪一个元素）
同时，可以使用`!important`强制提升优先级为最高，以便确定的使用。
...
使用`background-color`属性修改任意元素的背景颜色

#### 背景图片

使用`background-image`属性修改任意元素的背景图片
...
使用`background-repeat`属性控制背景图片的平铺方式
...
使用`background-image`属性，使用`linear-gradient()`函数来创建线性渐变

#### 多个背景图像

使用`background-image`属性，使用`,`分隔符来添加多个背景图像
...
使用`border`属性来设置边框，同[盒模型](README.md#盒边框)中的边框

#### 圆角

使用`border-radius`属性和与方框的每个角相关的长边来实现方框的圆角。可以使用两个长度或百分比作为值，第一个值定义水平半径，第二个值定义垂直半径。在很多情况下，您将只传递一个值，这两个值都将使用
